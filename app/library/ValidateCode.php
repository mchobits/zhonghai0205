...
class ValidateCode
{
    ...
    */
    ...
    private function writeFont()
      {
          $ttf = dirname(__FILE__) . '/font/elephant.ttf';
        $every = ceil($this->width / $this->verifyNums);
        for($i = 0;$i < $this->verifyNums;$i++){
          //$x = mt_rand(($every * $i) ,$every * ($i + 1) );
          $x = ($this->width - 16 * 4 * 1.2) * $i;
          //$y = mt_rand(5,$this->height - 15);
          $y = 15 * 1.5;
          imagettftext($this->res,15,0, $x, $y,$this->setLightColor(),$ttf,$this->verifyCode[$i]);
