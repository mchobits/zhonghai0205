...
class XCaptcha extends Component
{
    public $seKey    = 'ycf.direct.selling'; //验证码加密密钥
    public $expire   = 1800; // 验证码过期时间（s）
    public $useZh    = false; // 使用中文验证码
    public $useImgBg = false; // 使用背景图片
    public $fontSize = 32; // 验证码字体大小(px)
    ...
    public function entryCode($id = '')
    {
        ...
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/XCaptcha/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        $res = $this->session->set($key, $session);
