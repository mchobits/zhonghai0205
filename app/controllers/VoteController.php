...
//        $this->view->setVar('vote_count', $vote_records->count() + intval($candidates_count[0]->plus_vote));
        ...
        $vote_records = $this->modelsManager->executeQuery('SELECT SUM(vote_count) vote_counts FROM Candidates ');
        //$this->view->setVar('vote_count', intval($vote_records[0]->vote_counts) + intval($candidates_count[0]->plus_vote));
        $this->view->setVar('vote_count', intval($vote_records[0]->vote_counts));
...
//            }
            ...
            if ($this->end_time < (time() * 1000)) {
                return $this->response->setContent(json_encode(array(
                    'flag' => 103,
                    'msg' => '本投票已经结束了,详细评选结果公示请留意广东省民政厅官网！<br>感谢您对本次活动的支持和参与！'
                )));
            }
...
//        $candidate_vote_count = Candidates::find([
//            'conditions' => 'id = ?1',
//            'bind'       => [
//                1 => $real_id,
//            ]
